...
function Agent:init()
   ...
   -- Esta vaca come "grassEated" de media a cada paso
   ...
   self.hungryActive = self.grassEated*0.2;
   self.hungryPasive = self.grassEated*0.1;
   self.grassToManure = self.grassEated*0.4;

   io.write( 'init ' .. self.grassEated .. ' ' .. self.grassToHungry .. ' ' .. self.hungryActive .. ' ' .. self.hungryPasive .. ' ' .. self.grassToManure .. '\n')
end
...
function Agent:eatAndPoop(delta)
   ...
   if (grass > 0 and self.hungry < 80) then
